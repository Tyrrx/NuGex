...
When the MCP server starts, it SHALL recursively search its current working directory for `.sln` and `.slnx` files, ignoring `.csproj`/`.fsproj` files and skipping `bin`, `obj`, `.git`, `.vs`, and `node_modules` directories.
